...
def is_addressed_to_me(update: Update) -> bool:
    ...
    if BOT_NAME_LOWER in text_lower:
        # Не путаем с частью других слов
...
async def start_command(update: Update, context):
    ...
    await update.message.reply_text(
        ...
        f"• @{BOT_USERNAME}\n"
        f"• {BOT_NAME}\n\n"
        f"В группе я отвечаю только когда ко мне обращаются."
    )
...
async def handle_message(update: Update, context):
    ...
    # Убираем упоминания и имена
    user_text = re.sub(f"@{BOT_USERNAME}", "", user_text, flags=re.IGNORECASE)
    user_text = re.sub(r'\b' + re.escape(BOT_NAME_LOWER) + r'\b', "", user_text, flags=re.IGNORECASE)

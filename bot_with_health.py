...
async def editor_mode(update: Update, context):
    ...
    user_id = update.effective_user.id
    user_modes[user_id] = MODE_EDITOR
    await update.message.reply_text("📝 Режим редактора: редактирую тексты.")
...
async def conversation_mode(update: Update, context):
    ...
    await update.message.reply_text("💬 Режим общения: могу поговорить на любые темы.")
...
async def smart_mode(update: Update, context):
    ...
    await update.message.reply_text("🧠 Умный режим: сама определю.")
...
def main():
    ...
    print("=" * 50)
    print(f"{BOT_NAME} ЗАПУЩЕНА с HTTP сервером на порту 10000")
